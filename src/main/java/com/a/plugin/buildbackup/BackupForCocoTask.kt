...
abstract class BackupForCocoTask : DefaultTask() {


    // ./gradlew dailyDebugBackupForCoco  --toDir=~/xx/fxj --buildNum=100
    ...
    fun taskAction() {
        try {
            ...
            println("**** start BackupForCocoTask ****")
            println("toDir:${toDir}")
            val variantName: String = name.removeSuffix("BackupForCoco")
...
//        }
    ...
    private fun copyModuleProjectToDir(p: Project, variantName: String, destDir: File) {
        try {
            if (p.name.endsWith("-no-op")) return
